...
test('Works with custom port', async (t: Assertions) => {
	stdMock.use({ print });
	const { app, server } = await N9NodeRouting(ApplicationModule, { http: { port: 4002 } });
	stdMock.restore();
...
test('Works with preventListen = true', async (t: Assertions) => {
	stdMock.use({ print });
	const { app, server } = await N9NodeRouting(ApplicationModule, { http: { port: 4002, preventListen: true } });
	stdMock.restore();
	const output = stdMock.flush();
...
test('Works with custom log and should add a namespace', async (t: Assertions) => {
	...
	stdMock.use({ print });
	const { app, server } = await N9NodeRouting(ApplicationModule, { log });
	stdMock.restore();
	const output = stdMock.flush();
...
test('Works without options', async (t: Assertions) => {
	stdMock.use({ print });
	const { app, server } = await N9NodeRouting(ApplicationModule);
	stdMock.restore();
	const output = stdMock.flush();
...
test('Get app name on /', async (t: Assertions) => {
	stdMock.use({ print });
	const { server } = await N9NodeRouting(ApplicationModule, {});
...
test('Fails with PORT without access', async (t: Assertions) => {
	...
	const err = await t.throwsAsync(async () => {
		await N9NodeRouting(ApplicationModule, { http: { port: 80 } });
	});
	stdMock.restore();
	stdMock.flush();
...
test('Fails with PORT already used', async (t: Assertions) => {
	stdMock.use({ print });
	await N9NodeRouting(ApplicationModule, { http: { port: 6000 } });
	const err = await t.throwsAsync(async () => {
		await N9NodeRouting(ApplicationModule, { http: { port: 6000 } });
	});
	stdMock.restore();
...
test('Fails with PORT not in common range', async (t: Assertions) => {
	stdMock.use({ print });
	const err = await t.throwsAsync(async () => {
		await N9NodeRouting(ApplicationModule, { http: { port: 10000000 } });
	});
