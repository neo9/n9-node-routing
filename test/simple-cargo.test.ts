...
ava('Use a small cargo to compute data with default dispatch function', async (t: Assertions) => {
	stdMock.use({ print });
...
ava('Simple cargo with error inside worker', async (t: Assertions) => {
	stdMock.use({ print });
	const cargo = new Cargo<{ v: number; newV: number }, { v: number }>(
		'simple-fct-cargo',
...
ava('Simple cargo with error inside dispatch', async (t: Assertions) => {
	stdMock.use({ print });
	const cargo = new Cargo<{ v: number; newV: number }, { v: number }>(
		'simple-fct-cargo',
		async (requests) => {
