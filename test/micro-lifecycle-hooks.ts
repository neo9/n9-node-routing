...
ava('[Lifecycle Hooks] init in order', async (t: Assertions) => {
	...
	const output = stdMock.flush().stdout.filter(commons.excludeSomeLogs);
	// Logs on stdout
	t.true(output[0].includes('feature init 1'), 'feature init 1');
	t.true(output[1].includes('feature init after a long wait'), 'feature init after a long wait');
	t.true(output[2].includes('feature init 2'), 'feature init 2');
	t.true(output[3].includes('Listening on port 5000'), 'Listening on port 5000');
	t.true(output[4].includes('feature started 1'), 'feature started 1');
	t.true(output[5].includes('feature started after a long wait'), 'feature started');
	t.true(output[6].includes('feature started 2'), 'feature started 2');
